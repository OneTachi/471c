...
def get_free_variables(term: Term) -> set[Identifier]:
    match term:
        ...
        case Branch(operator=_operator, left=left, right=right, consequent=consequent, otherwise=otherwise):
            return get_free_variables(left) | get_free_variables(right) | get_free_variables(consequent) | get_free_variables(otherwise)
...
def dead_code_elimination_program(
    ...
    return Program(
        ...
        body=dead_code_elimination(program.body)
    )
