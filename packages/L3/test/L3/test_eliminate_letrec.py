...
def test_check_term_letrec_inside_bound():
    term = L3.LetRec(
        bindings=[
            ...
            ("y", L3.Primitive(operator="-", left=L3.Reference(name="x"), right=L3.Immediate(value=1))),
        ],
    ...
    expected = L2.Let(
        ...
        body=L2.Begin(
            effects=[
                ...
                L2.Store(
                    base=L2.Reference(name="y"),
                    index=0,
                    value=L2.Primitive(
                        operator="-", left=L2.Load(base=L2.Reference(name="x"), index=0), right=L2.Immediate(value=1)
                    ),
                ),
            ],
...
def test_check_term_immediate_basic():
    term = L3.Immediate(value=10)

    context: Context = {}

    expected = L2.Immediate(value=10)
...
def test_check_term_primitive_basic():
    term = L3.Primitive(operator="-", left=L3.Immediate(value=0), right=L3.Immediate(value=2))

    context: Context = {}

    expected = L2.Primitive(operator="-", left=L2.Immediate(value=0), right=L2.Immediate(value=2))
...
def test_check_term_load_basic():
    term = L3.Load(base=L3.Reference(name="x"), index=5)

    context: Context = {}

    expected = L2.Load(base=L2.Reference(name="x"), index=5)
...
def test_check_term_store_basic():
    term = L3.Store(base=L3.Reference(name="x"), index=2, value=L3.Reference(name="y"))

    context: Context = {}

    expected = L2.Store(base=L2.Reference(name="x"), index=2, value=L2.Reference(name="y"))
...
def test_check_term_begin_basic():
    term = L3.Begin(effects=[L3.Reference(name="x"), L3.Reference(name="y")], value=L3.Immediate(value=5))

    context: Context = {}

    expected = L2.Begin(effects=[L2.Reference(name="x"), L2.Reference(name="y")], value=L2.Immediate(value=5))
